...
class PMTreco:
    ...
    def get_waveform(self, outsize = 20, max_channels=32, view=False, truncate=False, verbose=False):
        ...
        #print(self.run)
        if self.run > 0:
            if(os.path.exists(self.path+'WFs_{:05d}.npy'.format(self.run))):
                wfs = np.load(self.path+'WFs_{:05d}.npy'.format(self.run), allow_pickle=True)
                
            else:
                mfile = self.getmid(self.run)
                count = 0
                waveform = []
                dgtz_header = []
                ...
                wfs = np.asarray(waveform, dtype = object)
                #print('ciao', flush = True)
                np.save(self.path+'WFs_{:05d}.npy'.format(self.run), wfs, allow_pickle=True, fix_imports=False)
                
                
        else:
            wfs = []
            for r in self.run_list:
                #print(r, self.path+'WFs_{:05d}.npy'.format(r),os.path.exists(self.path+'WFs_{:05d}.npy'.format(r)))
    ...
    def getWFNum(self):
        if self.run > 0:
            ...
            for i in range(self.WFs.shape[0]):
                tot_evs = tot_evs + self.WFs[i].shape[0]
            return tot_evs
        else:
    ...
    def getPedestal(self, pic_index, event_index, channel, run = 99999, pmax = 200):
        if self.run > 0 :
            return np.mean(self.WFs[pic_index][event_index][channel][:pmax])
        else:
    ...
    def getIntegral(self, pic_index, event_index, channel, run = 99999, int_min = 200, int_max = 400):
        if self.run > 0 :
            ped = self.getPedestal(pic_index, event_index, channel)
            yyyy = - (self.WFs[pic_index][event_index][channel]-ped)
    ...
    def plot_single_WF(self, pic_index, event_index, channels, save = False, run = 99999):
        ...
        if self.run > 0 :
            for ch in channels:
                ped = self.getPedestal(pic_index, event_index, ch)
                yyyy = (self.WFs[pic_index][event_index][ch]-ped)/4096 # hard coded???
                plt.plot(np.arange(0, len(yyyy), 1), yyyy, label = 'CH{:02d}'.format(ch))

            plt.xlabel('DGTZ sample')
            plt.ylabel('Waveform [V]')
            plt.legend()
            if save: plt.savefig(path+'plot_WF_{0:06d}_{1:06d}.jpg'.format(pic_index, event_index))
            plt.show()
    ...
    def getAllIntegrals(self, channel):
        if self.run > 0 :
            ints = np.zeros(self.WF_num)
            count = 0

            for i in range(self.pic_num):
                for j in range(len(self.WFs[i])):
                    ints[count] = self.getIntegral(i, j, channel)
                    count = count + 1
            return ints
        else:
        ...
        if self.run > 0 :
            ...
            for i in range(self.pic_num):
                for j in range(len(self.WFs[i])):
                    ...
                    for ch in channels:
                        ped = self.getPedestal(i, j, ch)
                        yyy = (self.WFs[i][j][ch] - ped) / 4096 # hard coded???
                    ...
                    selected.append(np.array([i, j, sel]))
